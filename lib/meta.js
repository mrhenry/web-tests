...
function meta(spec, section) {
	...
	if (spec === "geometry-1") {
		const index = mapping.geometry_1.bySection[section];
		if (typeof index !== 'number') {
			return;
		}

		return mapping.geometry_1.items[index];
...
function testSources(spec, section) {
	...
	if (spec === "geometry-1") {
		...
		const meta = mapping.geometry_1.items[index];
		const testSources = {};
		for (const key in meta.tests) {
			const testSource = fs.readFileSync(path.join(__dirname, '../', meta.path, meta.tests[key])).toString();
			testSources[key] = testSource;
...
function list(spec) {
	...
	if (spec === "geometry-1") {
		return mapping.geometry_1.items;
	}
